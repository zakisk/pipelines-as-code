...
func TestReportStatusCheckFromRepoSettings(t *testing.T) {
	tests := []struct {
		...
		unmatchedPRs         map[string]*tektonv1.PipelineRun
	...
	}{
		{
			...
			noMatchConclusion: "",
			unmatchedPRs: map[string]*tektonv1.PipelineRun{
				"pr-one": {ObjectMeta: metav1.ObjectMeta{GenerateName: "pr-one-"}},
			},
		...
		},
		{
			...
			unmatchedPRs: map[string]*tektonv1.PipelineRun{
				"pr-with-name": {ObjectMeta: metav1.ObjectMeta{Name: "pr-with-name"}},
			},
		...
		},
		{
			...
			unmatchedPRs: map[string]*tektonv1.PipelineRun{
				"pr-one":   {ObjectMeta: metav1.ObjectMeta{GenerateName: "pr-one-"}},
				"pr-two":   {ObjectMeta: metav1.ObjectMeta{GenerateName: "pr-two-"}},
				"pr-three": {ObjectMeta: metav1.ObjectMeta{Name: "pr-three"}},
			},
			expectedConclusion:  providerstatus.ConclusionSkipped,
		...
		},
		{
			...
			noMatchConclusion: "skipped",
			unmatchedPRs: map[string]*tektonv1.PipelineRun{
				"my-named-pr": {ObjectMeta: metav1.ObjectMeta{Name: "my-named-pr", GenerateName: "should-not-use-"}},
			},
			expectedConclusion:  providerstatus.ConclusionSkipped,
			expectedStatusCount: 1,
		},
		{
			...
			unmatchedPRs: map[string]*tektonv1.PipelineRun{
				"pr-fail": {ObjectMeta: metav1.ObjectMeta{GenerateName: "pr-fail-"}},
			},
			expectedStatusCount: 0,
